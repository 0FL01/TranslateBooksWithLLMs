...
def create_llm_client(
    ...
    Args:
        llm_provider: Provider type ('ollama', 'gemini', 'openai', 'openrouter', 'mistral', 'deepseek', 'poe', or 'nim')
    ...
    if llm_provider == "nim":
        # Use NIM-specific endpoint, not the passed api_endpoint (which defaults to Ollama)
        from src.config import NIM_API_ENDPOINT

        return LLMClient(
            provider_type="nim",
            api_endpoint=NIM_API_ENDPOINT,
            model=model_name,
            api_key=nim_api_key,
            context_window=context_window,
            log_callback=log_callback,
